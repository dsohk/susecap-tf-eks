...
resource "aws_iam_role" "susecap-node" {
  name = "susecap-ec2-role"
...
resource "aws_security_group_rule" "susecap-node-ingress-cluster" {
...
locals {
  susecap-node-userdata = <<USERDATA
#!/bin/bash
set -o xtrace
/etc/eks/bootstrap.sh --apiserver-endpoint '${aws_eks_cluster.susecap.endpoint}' --b64-cluster-ca '${aws_eks_cluster.susecap.certificate_authority.0.data}' '${var.cluster-name}'
USERDATA
}
